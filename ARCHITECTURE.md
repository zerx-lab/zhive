...
> 以下 clone 在本机的参考项目仅用于**设计借鉴与代码参考**，不作为依赖（红线：禁止引入）。

| 项目 | 本机路径 | 借鉴维度 |
|---|---|---|
| **codex** | `~/Desktop/code/github/codex/codex-rs` | 协议 / transport / 持久化 / 跨 thread fork（`core/src/thread_manager.rs::fork_thread`）/ 父子 permission delegate（`codex_delegate.rs`）蓝本 |
| **pi** | `~/Desktop/code/github/pi/packages` | 三注入队列、JSONL+Leaf 指针、extension manifest、`pendingSessionWrites` |
| **opencode** | `~/Desktop/code/github/opencode` | **TUI 交互**（见下），bun/TS + OpenTUI+SolidJS |

### opencode TUI 借鉴要点（zhive-tui 的 session 列表 + subagent 展示参考）

TUI 根目录：`packages/opencode/src/cli/cmd/tui/`（OpenTUI+SolidJS，非 Go bubbletea）。

- **session 列表 / resume**：`ui/dialog-select.tsx`（通用 fuzzy-finder list，session/model/timeline 全复用）+ `feature-plugins/session/{dialog,preview-pane}.tsx`（列表 + 实时预览双栏）。触发 `/sessions`（别名 `/resume`）+ leader 键；分组（Today / 日期 / Pinned）、当前会话 `●` 标记、`<leader>1..9` 快速切换、`Ctrl-D` 两段确认删除。
- **subagent = 带 `parentID` 的独立子 session**（不是 UI 嵌套渲染）：`tool/task.ts` 创建 `parentID` 子 session；父流里 `routes/session/index.tsx:2177-2267` 的 `<Task>` 组件渲染一行**活体摘要**（`✓ <Agent> Task — <desc> ↳ N toolcalls · 耗时`，实时订阅子 session 事件流），可点击/键控进入完整子会话视图；`routes/session/subagent-footer.tsx` 提供子会话页脚导航（`Parent ↑ · Prev ← · Next →` + 兄弟序号 + token/cost）。
- **其他高价值交互**：统一命令面板 + slash 注册表（`app.tsx` `appCommands`）、leader-key + which-key 提示、会话内 timeline 跳转 / fork（`routes/session/dialog-timeline.tsx`）、toast + 两段确认弹窗基建、token/cost + 上下文占比用量可视化（`subagent-footer.tsx`）。

**zhive 落地映射**：subagent 复制"子 thread + 父流活体摘要行"模型（zhive `ThreadHandle` 已有 `parent_thread_id`，纯事件驱动会话正好契合）；session 列表做一个通用 ratatui `SelectList` overlay 供 session/model 等复用。后续可直接借鉴上述 opencode 文件的实现代码。
