...
| 风险触发 | R-7 | 持久化采用 `sqlx = "0.8"`（`SqlitePool` × 4 库，原生 tokio async pool，0 自写 pool 代码） | B3 §6 / TODO B3-2 | ✅ sqlx SqlitePool |
...
| 风险触发 | R-6 | OTel 三 crate（`tracing-opentelemetry` / `opentelemetry` / `opentelemetry_sdk`）声明为 crate 侧 `optional=true`，gate 在默认关闭的 `otel` feature 后；字段命名按 OTel semconv 对齐 | B9 §3 / TODO B9-6 | ✅ optional dep + `otel` feature |
...
| 3.4 | tracing-opentelemetry layer | ✅ OTel 三 crate 声明为 crate 侧 optional dep，`#[cfg(feature = "otel")]` gate（默认关闭） |
| 3.5 | Subprocess hook executor | ✅ `HookExecutor::Subprocess` + `run_subprocess_hook` + `register_subprocess_hook` 已实装 |
...
**建议修订**：补两个容器 span，对应 EnginePhase 中两个尚未覆盖的态：
...
### 2.1 R-7: connection pool —— sqlx SqlitePool

**来源**：B3 §6 / TODO B3-2

持久化采用 `sqlx = "0.8"`（features `runtime-tokio, sqlite, migrate, macros, json`；不用 `query!` / `query_as!` 宏以免 `cargo check` 依赖 `DATABASE_URL`）。每个 DB 一个 `SqlitePool`（WAL + `Synchronous::Normal`），由 sqlx 原生 tokio async pool 管理连接，无需自写 pool 代码，也避免了跨 `await` 持锁的串行写瓶颈。`sqlx::migrate!("./migrations/<db>")` 宏直接内嵌 4 库迁移 SQL。

sqlx 作为新依赖在 `Cargo.toml` 落地（root + zhive-core 均 `{ workspace = true }`）。

---

### 2.2 R-8: 跨库一致性（已自决，方案已选）
...
### 2.4 R-4: llmsdk provider 适配（未触发）
...
### 2.5 R-6: tracing-opentelemetry —— optional dep + `otel` feature

**来源**：B9 TODO B9-6

OTel 三 crate（`tracing-opentelemetry` / `opentelemetry` / `opentelemetry_sdk`）在 root `Cargo.toml` 的 `[workspace.dependencies]` 平直声明（Cargo 无法在 workspace 层标 optional），在 zhive-core 的 `[dependencies]` 标 `optional = true`，并 gate 在默认关闭的 `otel` feature 后（`otel = ["dep:tracing-opentelemetry", "dep:opentelemetry", "dep:opentelemetry_sdk"]`）。默认构建不拉入 OTel，开启 feature 即接入 layer。字段命名按 OTel semconv 对齐，确保接入时零 wire 改动。

---

### 2.6 红线 1 触发待审：新增 dependency
...
- `events/session_aborted` notification（payload 含 `clearedSteer / clearedFollowUp / nextTurnRetainedCount`）
...
- `events/turn_suspended` notification（server → client；defer 触发，payload 含 `requestId / suspendedAt / reason?`）
- `events/turn_resumed` notification（server → client；resume 后续 turn）

### 3.3 B1 engine 状态机相关

- `events/phase_changed` notification（method 名）+ `PhaseChangedNotification { from, to, thread_id }` payload（A1 草图中缺，B1 §4 末尾补）
...
| 4 | tracing-opentelemetry layer | B9 §5.3 / TODO B9-6 | OTel 三 crate 声明为 crate 侧 optional dep；`#[cfg(feature = "otel")]` gate，默认关闭 |
| 5 | Subprocess hook executor | B5 §0 摘要 + TODO B5-9 | `HookExecutor { InProcess(BoxedHookFn), Subprocess(...) }` 双轨均已实装（`run_subprocess_hook` + `register_subprocess_hook`，带测试） |
...
| B3 | 8 | **B3-2 R-7 pool 选型**（§2.1）；B3-1 memories.jsonl 是否落盘；B3-5 zhive-core 单 crate 编译时间 |
| B4 | 12 | **B4-2 红线 2 unsafe**（§2.7）；B4-3 Windows transport 占位；B4-4 ClientRequest 放 proto 还是 core |
| B5 | 9 | **B5-2 jsonschema dep**（§2.6）；B5-3 futures dep；B5-7 串行 vs 并行分歧；B5-8 PhaseTransition 第 15 事件 |
...
| B9 | 7 | **B9-1 / B9-2**（zhive.compaction / branch_summary 容器 span，§1.14）；**B9-6 OTel 红线 1**（§2.5）；B9-3 phase span 利弊 |
...
1. ✅ R-7：持久化采用 `sqlx = "0.8"`（`SqlitePool` × 4 库）
2. ✅ R-8：JSONL source of truth + DB 异步重建 落定
3. ✅ D-006 8→14 case 采纳
4. ✅ D-008 三队列修订采纳
5. ✅ D-012 第 15 事件 PhaseTransition 采纳
6. ✅ D-013 三 namespace 修订（R-D013）采纳
7. ✅ 红线 1 / 红线 2：jsonschema 走 cargo add；futures 用现有 workspace dep；rustix 不引入；OTel 三 crate 标 crate 侧 optional + `otel` feature gate
...
2. **第二轮 - 风险触发 & 红线触发**（§2 共 7 项）：
   - ✅ R-7：持久化采用 `sqlx = "0.8"`（`SqlitePool` × 4 库，原生 tokio async pool）
   - ✅ R-8：JSONL source of truth 已选
   - ✅ R-3 / R-4：已落槌（tower-lsp 不作蓝本 / llmsdk 全覆盖）
   - ✅ R-6：OTel 三 crate 标 crate 侧 optional + `otel` feature gate（默认关闭）
...
| 6 | B1-engine-loop.md | 872 | Engine actor + EnginePhase 5 态 + Submission/Event 拓扑 |
| 7 | B2-state-memory-model.md | 501 | 活跃 Thread/Turn/Item 内存表征 + lazy load |
| 8 | B3-persistence.md | 782 | sqlx 4 库 + JSONL Leaf rollout + R-7/R-8 |
...
| B1 §0 | 设计衔接警告 | A1 TurnStatus 4 态 ⊥ B1 EnginePhase 5 态正交共存，**不改 A1** |
| B3 §0 | 风险触发 | R-2 触发（cold release 78-80s）+ R-7 等用户拍 + R-8 fail-strategy |
| B5 顶 ×3 | 决策冲突警告 | Claude Code 并行 vs zhive 串行（§1 / §2.6）；A4 PreToolUse mutate 后 abort turn（不回滚）；B1 PhaseTransition 待 A4 修订 |
