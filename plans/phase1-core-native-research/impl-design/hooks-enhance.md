# Hooks 增强：Unknown deserialize 降级 + 每-hook timeout 隔离 + Subprocess 执行 + signal/CancellableHook
...
(c) Subprocess 执行（zhive-core/hooks/mod.rs + hooks/subprocess.rs）。引入 `enum HookExecutor { InProcess(Arc<dyn HookFn>), Cancellable(Arc<dyn CancellableHookFn>), Subprocess(Arc<SubprocessSpec>) }`，HookRegistration.callback 改 `executor: HookExecutor`。三条执行路径全实装：InProcess 走 catch_unwind 隔离，Cancellable 把 signal 透传给 hook，Subprocess 调 `run_subprocess_hook` spawn 外部程序。子进程协议（对齐 Claude Code）：把 wire `HookEvent` 序列化喂子进程 stdin 后关闭 stdin（reader 见 EOF）；exit `0` 把 stdout 解析为 `HookOutput`（空 stdout = 无意见，非 JSON stdout 忽略），exit `2` 为 blocking 信号，其他 exit code 或被信号终止当非阻塞失败处理；超时由 `register_subprocess_hook` 的 `timeout` 决定，`None` 用 `DEFAULT_SUBPROCESS_TIMEOUT`（600s），到点用 `kill_on_drop` 杀子进程。SubprocessSpec 经 `register_subprocess_hook` 入口注册。为最小化爆炸面：保留现有 `register(...callback: Arc<dyn HookFn>)` 签名内部包成 `HookExecutor::InProcess`，对外零破坏。
...
- `crates/zhive-core/src/hooks/mod.rs` — HookRegistration 加 timeout:Option<Duration> + 把 callback:Arc<dyn HookFn> 改为 executor:HookExecutor；新增 enum HookExecutor{InProcess(Arc<dyn HookFn>),Cancellable(Arc<dyn CancellableHookFn>),Subprocess(Arc<SubprocessSpec>)}；新增 trait CancellableHookFn + 对 Arc<dyn HookFn> 的 blanket adapter（忽略 signal）；HookHost::register 内部包 InProcess+timeout=None 保持旧签名零破坏，另加 register_with_timeout / register_cancellable_hook / register_subprocess_hook 入口；dispatch_inner 重写为 dispatch_with_signal：tokio::select! cancel 短路 + 每 hook tokio::time::timeout 包裹 catch_unwind，timeout/panic 都 warn+skip+continue；旧 dispatch(event) 用 never-cancelled 哨兵 token 委派 with_signal 版；补 timeout_isolated / signal_short_circuits / subprocess 执行测试
- `crates/zhive-core/src/hooks/subprocess.rs` — SubprocessSpec{program,args,...} + DEFAULT_SUBPROCESS_TIMEOUT + SubprocessHookError；run_subprocess_hook spawn 子进程，序列化 HookEvent 喂 stdin、读 stdout、按 exit-code 解析 HookOutput，超时 kill_on_drop
...
- crates/zhive-core/src/hooks/mod.rs: `pub enum HookExecutor { InProcess(Arc<dyn HookFn>), Cancellable(Arc<dyn CancellableHookFn>), Subprocess(Arc<SubprocessSpec>) }`（手写 Clone/Debug，因 trait object 不可 derive）
- crates/zhive-core/src/hooks/subprocess.rs: `pub struct SubprocessSpec { pub program: String, pub args: Vec<String>, ... }` + `pub const DEFAULT_SUBPROCESS_TIMEOUT: Duration` + `pub enum SubprocessHookError`
- crates/zhive-core/src/hooks/subprocess.rs: `pub(crate) async fn run_subprocess_hook(...) -> Option<HookOutput>`（spawn 子进程、JSON stdin/stdout、exit-code 语义、timeout kill_on_drop）
...
- crates/zhive-core/src/hooks/mod.rs: `pub fn HookHost::register_with_timeout(...)`（给现有 register 加 timeout 入口，破坏面小）
- crates/zhive-core/src/hooks/mod.rs: `pub fn HookHost::register_cancellable_hook(...)`（注册 Cancellable 执行器）
- crates/zhive-core/src/hooks/mod.rs: `pub fn HookHost::register_subprocess_hook(...)`（注册 Subprocess 执行器 + SubprocessSpec）
...
- SubprocessSpec 与 A5 manifest entrypoint：Phase 1 manifest entrypoint 仅 builtin（decision-diffs.md:35/267），故 manifest loader 不会构造 Subprocess；Subprocess 执行器经 `register_subprocess_hook` 编程式注册，子进程协议（JSON stdin/stdout + exit-code）已落地。
...
- core unit：subprocess_executor_runs（注册 Subprocess + 一个回 JSON 的程序 → dispatch spawn 子进程、解析 stdout 为 HookOutput；exit 2 = blocking、超时 kill 各覆盖）
...
3. (c) Subprocess：实装多臂 HookExecutor（InProcess/Cancellable/Subprocess）+ `register_subprocess_hook` 入口 + `run_subprocess_hook` 执行体（spawn 子进程、JSON stdin/stdout、exit-code 语义、timeout kill_on_drop），放进独立 `hooks/subprocess.rs`。
...
范围建议：(a)(b)(c)(d) 本阶段均做到可用+测试齐全。所有新公开 API 必须带 doc+doctest（红线）。每步独立 `cargo check -p <crate> --lib` + `cargo nextest run -p <crate>`，最后 fmt+clippy -D warnings。
