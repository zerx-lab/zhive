...
## currentState
crates/zhive-client-native/src/lib.rs:1-766 — Client 已实现为 Arc 克隆模型，有 connect_uds/connect_stdio/from_split/call/notify/subscribe_events/set_reverse_handler/negotiated_version/server_capabilities/server_info/cancel_turn。Client.shutdown() 当前在 lib.rs:459-462 仅同步调用 self.inner.shutdown.cancel() + drop(self.outbound_tx)，**无 timeout 兜底、无等待 worker**。

crates/zhive-client-native/src/connect.rs:67-120 — perform_handshake() 硬编码握手参数：client_info.name="zhive-client-native"(行68-77)，无 ClientBuilder。connect_uds(path) 行183、connect_stdio() 行197 均直接调 from_split + perform_handshake。

cancel 机制：cancel_turn() 在 lib.rs:427-450 走私有 engine/cancel_turn RPC（engine-side Request），与 ACP session/cancel notification 无关。**无 cancel_session() 方法**。

ClientBuilder：**不存在**。Cargo.toml 无 tokio::time feature（shutdown timeout 需要它）。

zhive-tui/src/rpc.rs:64-65 用 client.cancel_turn()；crates/zhive-cli/src/engine_host.rs:100 用 Client::connect_uds()、行131 用 client.clone().shutdown()。这两个调用点要保持向后兼容。
...
- zhive-core/server/handlers.rs：cancel_session 发出的 session/cancel notification 在 server 侧走 dispatch_message → router.dispatch_notification，当前 router 无 session/cancel handler（serve_loop.rs 路由逻辑），通知被静默忽略。B7 landing 需注册 session/cancel notification handler（在 register_engine_handlers 补注册，调 engine.cancel_turn(thread_id)），届时 cancel_session 才真正生效
...
3. cancel_session 与 cancel_turn 共存的 API 混淆：需要 doc comment 明确说明区别——cancel_turn 是 engine-private RPC，cancel_session 是 ACP 协议 notification。Phase1 server 不处理 session/cancel notification（B7 尚未 land），这必须在 doc comment 里注明 "Phase 1 server requires B7 to take effect"。
...
- Server 侧 session/cancel handler（B7）不在本 Block C 范围内，Phase1 只补 client 侧
